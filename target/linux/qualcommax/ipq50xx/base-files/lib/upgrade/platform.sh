...
glinet_do_fit_upgrade() {
	...
	local oem_revert=/tmp/oem_revert
	local label=ubi
	# revert to oem firmware via dual boot img
	if [ -s $oem_revert ]; then
		revert=$(cat $oem_revert)
		if [ ${revert} -eq 1 ]; then
			label=oem
		fi
	fi
	local part=$(dumpimage -l /tmp/firmware.bin | grep -o "Image [0-9] ($label)" | cut -f2 -d" ")

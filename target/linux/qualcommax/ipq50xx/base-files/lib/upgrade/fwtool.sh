...
fwtool_check_image() {
	...
	for k in $dev_keys; do
		...
		if [ "$dev" = "$device" ] || [ "$dev" = "$gl_oem" ]; then
			...
			if [ "${devicecompat#.*}" != "${imagecompat#.*}" ] || [ "$dev" = "$gl_oem" ] && [ "$SAVE_CONFIG" = "1" ]; then
				[ "$IGNORE_MINOR_COMPAT" = 1 ] && return 0
				if [[ ! -z "$gl_oem" ]]; then
					local tmp_dcompat="${devicecompat}"
					local tmp icompat="${imagecompat}"
					device_cur="OpenWrt Firmware"
					device_oem="GL-iNet OEM Firmware"
					v "The device is supported, but the config is incompatible to the new image { -- from -->[ $device_cur ] -- to --> [ $device_oem ] --> }. Please upgrade without keeping config (sysupgrade -n)."
					[ -n "$compatmessage" ] && v "$compatmessage"
					return 1
				else
